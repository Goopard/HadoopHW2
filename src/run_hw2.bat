...
ssh root@localhost -p 2222 command "python3 /usr/jobs/auto_test/test.py -r hadoop hdfs:///user/raj_ops/test.txt --hadoop-streaming-jar=/usr/hdp/2.6.4.0-91/hadoop-mapreduce/hadoop-streaming.jar > /usr/jobs/auto_test/output.txt --snappy -o hdfs:///user/raj_ops/output"


ssh root@localhost -p 2222 command hadoop fs -copyToLocal /user/raj_ops/output/* /usr/jobs/auto_test/output/
scp -P 2222 root@localhost:/usr/jobs/auto_test/output.txt output.txt
scp -P 2222 root@localhost:/usr/jobs/auto_test/output/* ../output/
